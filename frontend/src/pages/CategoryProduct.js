       ...
       <div className='hidden lg:grid grid-cols-[200px,1fr]'>
        ...
        <div className='bg-white p-2 min-h-[calc(100vh-120px)] overflow-y-scroll'>
          ...
          <div>
           ...
           {/**Filter */}
           <div>
            <h3 className='text-base uppercase font-medium text-slate-500 pb-1 border-b border-slate-300'>Filter</h3>
            <form className='text-sm flex flex-col gap-2 py-2'>
             {
              productCategory.map((categoryName,index)=>{
                return(
                  <div className='flex items-center gap-3'>
                    <input type='checkbox' name={'category'} checked={selectCategory[categoryName?.value]} value={categoryName?.value} id={categoryName?.value} onChange={handleselecteCategory}/>
        ...
        {/**Right side */}
        <div className='px-4'>
          <p className='text-lg font-medium text-slate-800 my-2'>Search results : {data.length}</p>
